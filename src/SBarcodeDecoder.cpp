...
void SBarcodeDecoder::process(QImage capturedImage, ZXing::BarcodeFormats formats)
{
    setIsDecoding(true);

    /*int new_height = capturedImage.height() * 5;
    int new_width = capturedImage.width() * 5;

    capturedImage = capturedImage.scaled(new_width, new_height, Qt::KeepAspectRatio);

    qDebug() << "captured width: " << capturedImage.width() <<"\n";
    qDebug() << "captured height: " << capturedImage.height() <<"\n";*/

...
public:
                    ...
                    QVideoFilterRunnable::RunFlags flags) override
    {
        ...
        QImage croppedCapturedImage =
                SBarcodeDecoder::videoFrameToImage(*input,_filter->captureRect().toRect());

//        int new_height = croppedCapturedImage.height() * 3;
//        int new_width = croppedCapturedImage.width() * 3;

//        croppedCapturedImage = croppedCapturedImage.scaled(new_width, new_height, Qt::KeepAspectRatio);

//        qDebug() << "croppedCapturedImage width: " << croppedCapturedImage.width() <<"\n";
//        qDebug() << "croppedCapturedImage height: " << croppedCapturedImage.height() <<"\n";
